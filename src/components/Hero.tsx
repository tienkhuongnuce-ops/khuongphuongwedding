  ...
  return (
    <section className="relative w-full h-screen min-h-[700px] flex items-center justify-center overflow-hidden bg-gray-900">
      ...
      <div className="relative z-10 text-center px-4 w-full max-w-6xl">
        <div className="mb-6 font-sans uppercase tracking-[0.3em] text-xs md:text-sm text-white border border-white/50 bg-black/20 backdrop-blur-md py-2 px-8 rounded-full inline-block shadow-lg">
          We Are Getting Married
        </div>
        
        <div className="flex flex-col items-center justify-center gap-4 mb-8">
           {/* Floating Names - White */}
          <h1 className="font-names text-6xl md:text-8xl lg:text-9xl text-white animate-float drop-shadow-2xl" style={{ animationDuration: '4s' }}>
            {groom.firstName}
          </h1>
          {/* Double Happiness Symbol - White */}
          <span className="font-serif text-5xl md:text-6xl text-white my-2 drop-shadow-lg select-none">囍</span>
          <h1 className="font-names text-6xl md:text-8xl lg:text-9xl text-white animate-float drop-shadow-2xl" style={{ animationDuration: '5s' }}>
            {bride.firstName}
          </h1>
        </div>

        {/* Perfectly Aligned Date Section - Text White, Borders White */}
